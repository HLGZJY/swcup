...
- admin 后台"事件列表"出现 1 条 `pending` 状态事件,类型=report
  ...
  新增的动物记录，位置并不是我自定义的，而是我的实际地址位置，这也是大问题。其次就是 这只金毛的信息是 年龄未知，健康未知，未绝育
...
实际结果： 实际得分 70分 鼻纹相似度100% GPS NULL 文本匹配相似度100% 。在底下出来的是“未在数据库中匹配到动物”让我创建档案 ，距M 走失中。
...
- user2 个人页"我的上报"新增 1 条 通过
- **系统自动合并**:A1 的事件数 +1,新事件 没有增加`status=duplicated`,`duplicate_of=<A1 animal_id>`,`fusion_score > 0.8`
- 首页"全部" tab 豆豆卡片显示"已 2 次上报"标记 没有显示
- admin 后台"事件合并"页应能看到这次合并记录 后台出现但是 鼻纹采集和事件上报位置不一样应该不会合并 。
