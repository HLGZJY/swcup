...
**所有账号复用现有 `backend/seed.py` 中的账号,密码统一 `password123`**

| 角色 | 账号 | 现有昵称 | 在测试中的定位 | 关键场景 |
|---|---|---|---|---|
| **admin** | 13900000001 | 管理员A | 事件审核、认领审批、状态流转 | S8, S9, S10 |
| **user1** | 13800000002 | 李明 | 走失主人(A1) + 鼻纹采集者(A1) | S1, S2 |
| **user2** | 13800000003 | 王小红 | 走失主人(A3) + 同区发现者(A2) | S3, S4 |
| **user3** | 13800000004 | 张小华 | 跨区发现者(A4) + 鼻纹采集者(A7) | S5, S6 |
| **user4** | 13800000005 | 陈建国 | 走失主人(A5/A6) + 待认领申请者(A8) | S6, S7 |
| **user5** | 13800000006 | 刘秀英 | 跨物种测试者(A9) + 跨区发现者(A10) | S9, S10 |

> 备注:现有 seed.py 已经有 user1-user7 + admin + 2 org,无需新建。user6/user7/org1/org2 在本测试中不参与,可不录入对应数据。
