...
│  [事件类型标签] [状态标签]        │  ← 事件头
│  描述文字...                      │
│  [icon-location] 地址  [icon-clock] 时间  │
│  [照片缩略图1] [照片缩略图2]       │
├─────────────────────────────────┤
│  AI 比对结果                      │  ← 四维度说明
│  向量相似度  40%  ████████░░ 0.72 │  ← CSS进度条，主色#0FBF9F填充
│  位置接近度  20%  ██████░░░░ 0.55 │
│  图像相似度  20%  ███████░░░ 0.68 │
│  文本匹配度  20%  ████░░░░░░ 0.41 │
│  ────────────────────────────    │
│  融合得分        ██████████ 0.64 │  ← 粗体强调，背景色块
├─────────────────────────────────┤
│  比对候选（3个）                  │  ← 候选列表（白色卡片+阴影）
│  ┌─────────────────────────────┐  │  ← card: bg:#FFF, radius:16rpx, shadow
│  │ [照片] 品种: 柴犬           │  │
│  │        状态: 走失中        │  │
│  │        融合: 0.81 [推荐]  │  │  ← badge: bg:#FF6B6B, color:#FFF
│  │        ( )选择              │  │  ← radio单选
│  └─────────────────────────────┘  │
│  ┌─────────────────────────────┐  │
│  │ [照片] 品种: 田园犬          │  │
...
| 元素 | 样式规范 |
|------|----------|
| 图标 | SVG icons（与项目统一，使用 `/static/icons/` 下现有图标） |
| 四维度进度条 | CSS `progress-bar`，填充色 `#0FBF9F`，背景 `#E8FDF8` |
| 融合得分行 | 加粗，背景色块突出（`background: #0FBF9F22`） |
| 候选卡片 | `background: #FFFFFF; border-radius: 16rpx; box-shadow: 0 2rpx 12rpx rgba(0,0,0,0.06)` |
| 推荐徽章 | `background: #FF6B6B; color: #FFFFFF; border-radius: 8rpx; padding: 4rpx 12rpx` |
| 选中态 | 卡片 `border: 2rpx solid #0FBF9F`，radio 高亮 |
| 照片缩略图 | `border-radius: 12rpx; object-fit: cover` |
