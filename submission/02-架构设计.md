...
| 章节 | 内容 | 对应代码 |
|------|------|---------|
| §1 | 架构总览：5 层 + 2 中台（评论 AI + 事件模型） | 本文档 §1 |
| §2 | 技术栈选型（已实际使用）| `backend/package.json`、`ai-service/requirements.txt` |
| §3 | 后端 9 模块分解（v1.1 +comments 模块）| `backend/src/app.module.ts` |
| §4 | AI 服务 4 端点 | `ai-service/src/main.py` |
| §5 | 数据层（6 表） | `backend/src/*/entities/*.entity.ts` |
| §6 | 4 个核心时序图（采集/上报/认领/intent 派生 v1.1） | `nose.service.ts`、`events.service.ts`、`admin.service.ts`、`animals.service.ts` |
| §7 | 关键设计决策（9 项 trade-off，v1.1 +2） | 各处代码注释 |
...
### 1.1 5 层架构图（v1.1，4 层扩展 + 1 中台）

![鼻纹智救 · 5 层系统架构](./assets/architecture/5-layer-arch.png)

> **架构原则**：客户端 → 管理端 → 评论 AI 中台 → 统一事件模型中台 → AI 服务 → 数据库，**单向依赖**；业务逻辑集中在 NestJS 后端（第 2 层），AI 服务只做"纯计算"无状态；MySQL 是唯一持久化层。详细模块职责见 §3。

**v1.1 新增的两个中台**：
- **评论 AI 中台**：4 个 Python 模块（moderate / summary / clue_matcher / dict_loader）+ 6 个 JSON 词典 + ai-bridge 3 种模式（http/stub/hybrid，默认 hybrid）
- **统一事件模型中台**：intent 字段驱动 status 派生 + admin 4 动作派发（reject/confirm/merge/create_new）

### 1.2 架构选型原则（6 个，v1.1 +2）

1. **分层清晰**：客户端 → 网关 → 评论 AI → 事件模型 → AI → 数据库，**单向依赖**，不允许反向调用。
2. **业务在网关层**：所有业务逻辑（融合评分、状态流转、评论审核）都在 NestJS 后端，AI 服务只做"纯计算"。
3. **AI 服务无状态**：FastAPI 进程重启不影响业务，重启时从磁盘加载 `.pth` 权重。
4. **数据唯一源**：MySQL 是唯一持久化层，AI 服务不存任何业务数据（只读 `.pth` 文件）。
5. **中台可降级（v1.1 新增）**：评论 AI 中台 ai-bridge 三种模式（http/stub/hybrid），AI 服务不可用时自动降级 stub（内插词典兜底）
6. **事件模型向后兼容（v1.1 新增）**：intent 字段是 DTO 业务层概念（**不入库**），老数据无 intent 时按 `event_type` 派生
...
@Module({
  imports: [
    ...
    TypeOrmModule.forRoot({
      type: 'mysql',
      entities: [User, Animal, NoseFeature, RescueEvent, Claim, Comment],  // ← 6 个 entity (v1.1 +Comment)
...
**注意**：
- `MatchingModule` 在 `backend/src/matching/` 目录中存在但**未在 `app.module.ts` 中注册**。这是有意的——`MatchingService` 仍可被 `EventsService` 通过 `import` 直接调用（NestJS 允许跨模块 service 调用），无需注册 HTTP 控制器。
- v1.1 新增 `CommentsModule`，对应 6 张表中的 `comments` 表与评论 AI 中台。

### 3.2 9 模块职责矩阵（v1.1 +CommentsModule）
...
| **Animals** | `src/animals/` | `animals.controller.ts` | `animals.service.ts` | 动物档案 CRUD + 时间轴 | 7 |
| **Nose** | `src/nose/` | `nose.controller.ts` | `nose.service.ts` + `nose-text-match.ts` | 鼻纹采集/比对/品种 | 3 |
| **Events** | `src/events/` | `events.controller.ts` | `events.service.ts` | 救助事件（含 intent 派生 + 用户自助关联） | 4 |
| **Claims** | `src/claims/` | `claims.controller.ts` | `claims.service.ts` | 认领申请 | 2 |
| **Comments**（v1.1 新增）| `src/comments/` | `comments.controller.ts` | `comments.service.ts` | 评论 + AI 审核 + 摘要 | 3 |
| **Upload** | `src/upload/` | `upload.controller.ts` | - | 文件上传（Multer）| 1 |
| **Admin** | `src/admin/` | `admin.controller.ts` | `admin.service.ts` | 管理端（统计/审核/CRUD/线索）| 19 |

**合计：47 个 REST 端点 + 4 个 AI 端点 = 51 个对外 API**（v1.0 是 42+4=46，v1.1 新增 5 个端点）。
...
└──────┬──────┘         └─────────────┘
       ...
       ▼                             ▼
┌─────────────┐               ┌─────────────┐
│rescue_event │ N──────1 (admin 审核)│  comment   │ (v1.1 新增)
│ (救助事件)  │               │ (评论)      │
└──────┬──────┘               └─────────────┘
       │ 1 (可选)
       ▼
...
### 6.4 时序图 4（v1.1 新增）：intent 驱动的状态派生

![时序图 4 · intent 派生](./assets/architecture/seq-intent.png)

> 上图可视化展示，ASCII 时序细节保留如下：

```
用户       小程序         后端 NestJS          评论 AI 中台     MySQL
 │          │               │                   │              │
 │ 1. 上报事件│               │                   │              │
 │   intent='lost'│            │                   │              │
 │   body + GPS  │            │                   │              │
 ...
 │          │               │   → 自动建档      │              │
 │          │               │                   │              │
 │          │               │ 3. INSERT animal (status=LOST)     │
 │          │               │─────────────────────────────────→│
 │          │               │ 4. INSERT event (animal_id=新ID)  │
 │          │               │─────────────────────────────────→│
 │          │               │                   │              │
 │ 5. 用户发布评论│            │                   │              │
 │          │ POST /v1/comments               │              │
...
**intent → status 派生规则**（`backend/src/animals/animals.service.ts:103-107`）：

```typescript
status: dto.intent === 'found' ? AnimalStatus.FOUND : AnimalStatus.LOST,
```

**admin 4 动作派发**（`backend/src/admin/admin.service.ts:177-225` `dispatchEventAction`）：

| action | event.status 终态 | animal_id 来源 | UI 语义 |
|--------|-------------------|----------------|---------|
| `reject` | `rejected` | 不变 | 驳回 |
| `confirm` / `merge` | `duplicated` | 必须传 | 合并到现有动物 |
| `create_new` | `confirmed` | 从 event 字段建 Animal | admin 手动建档 |

**参考**：[09-B线改动-状态流转参考.md](09-B线改动-状态流转参考.md) §1.2 / §1.3 / §1.4（状态机总览图）。
...
### 决策 D8（v1.1 新增）：intent 字段是 DTO 业务层概念，不入库

**做法**：`intent` 字段加在 `CreateAnimalDto` 和 `CreateEventDto`，**不入库**（不入 `animals` 表也不入 `rescue_events` 表）。状态派生和事件分发都在 service 层基于 intent 完成。

**权衡**：

| 方案 | 优点 | 缺点 |
|------|------|------|
| ✅ DTO 层（当前）| 老数据无需迁移，向后兼容 | 无法 SQL 查询 intent 分布 |
| ✗ 入库 | 可查询、可分析 | 老数据需回填，破坏 schema |

**理由**：B 线阶段 3 没有 migration，回填老数据需要单独脚本；阶段 4 才考虑把 intent 入库。

### 决策 D9（v1.1 新增）：评论 AI 中台用规则 + 词典 + 阈值，不用 LLM

**做法**：4 个 Python 模块（moderate / summary / clue_matcher / dict_loader）+ 6 个 JSON 词典 + 阈值判定，**全程不调任何 LLM**。

**权衡**：

| 方案 | 优点 | 缺点 |
|------|------|------|
| ✅ 规则 + 词典（当前）| 0 GPU 成本，0 LLM API 成本，可降级 | 词典覆盖率有限，需人工维护 |
| ✗ LLM (Qwen / GPT) | 语义理解强 | 需 GPU + API 成本，每次调用 ~1 元 |

**理由**：MVP 阶段优先成本可控，规则 + 词典已能覆盖 80% 场景；后期可平滑接入 LLM 做兜底。
...
- ✅ 5 层分离（前端/网关/评论 AI 中台/事件模型中台/AI/数据）
- ✅ 47 REST + 4 AI 端点（v1.1 +5）
- ✅ 6 表关系建模（v1.1 +comments）
- ✅ JWT 鉴权 + bcryptjs
- ✅ 2 套融合算法（采集/上报）
- ✅ intent 派生 + admin 4 动作派发（v1.1）
- ✅ 评论 AI 中台（moderate/summary/clue_matcher/dict_loader，v1.1）
