...
> **文档版本**：v1.0（2026-06-13，与代码库 `nose_v3_sgd.pth`/`breed_classifier_v3.pth` 一致）
...
| §11 | 风险与备选方案 | FACT-BASELINE §D 已知缺陷 |
| §12 | 致评审老师：可复现的 5 分钟验证清单 | `appendix/DEPLOY.md` |
...
[ 输入图片 224×224×3 ]
        │
        ▼
[ ResNet50 ImageNet 预训练 backbone ]
        │   (conv1 → bn1 → relu → maxpool → layer1..4 → avgpool → flatten)
        │   输出：2048-d feature
        ▼
[ Dropout(p=0.5) ]   ← 防过拟合（mobilenet.py:87）
        │
        ▼
[ Linear(2048, 512) ]   ← backbone.fc 替换（mobilenet.py:84）
        │
        ▼
[ L2 Normalize(dim=1) ]  ← 强制归一化到单位球面（mobilenet.py:104）
        │
        ▼
[ 512-d Embedding ]
        │
        ├─[ 部署模式 ]→ 直接返回向量
        │
        └─[ 训练模式 ]→ ArcMarginProduct(512, num_classes, s, m)
                              │
                              ▼
                        [ 157 类 / 6000 类 softmax logits ]
```
...
### 11.1 已记录在 FACT-BASELINE §D 的 4 类问题
