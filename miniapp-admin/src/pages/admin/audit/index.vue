  ...
  <view class="page">
    ...
    <view class="hero">
      ...
      <view class="hero-content">
        ...
        <view
          class="hero-stat"
          :class="pendingTotal > 0 ? 'has-pending' : 'all-clear'"
        >
          <text class="stat-num">{{ pendingTotal }}</text>
          <text class="stat-label">{{
            pendingTotal > 0 ? "待审" : "已清"
          }}</text>
        </view>
      </view>
    </view>
        ...
        <view class="claim-content">
              ...
              <view class="user-avatar">
                <image
                  ...
                  :src="
                    ...
                    '/static/mock/avatar-default.png'
                  "
                  mode="aspectFill"
                />
              </view>
          ...
          <view class="claim-animal">
            <image
              class="animal-thumb"
              :src="
                resolveImageUrl(item.animal?.photos?.[0]) ||
                '/static/mock/dog-placeholder.png'
              "
              mode="aspectFill"
            />
            <view class="animal-meta">
              <text class="animal-breed">{{
                item.animal?.breed || "未知品种"
              }}</text>
              <view class="animal-info-row">
                <text class="animal-info-item"
                  >📍 {{ item.animal?.address || "未知地点" }}</text
                >
              </view>
              <view class="animal-info-row">
                <text class="animal-info-item" v-if="item.animal?.color"
                  >🎨 {{ item.animal?.color }}</text
                >
              </view>
            </view>
          </view>
          ...
          <view class="claim-actions">
            <view
              ...
              @click="onRejectClaim(item.claim_id)"
            >
              <image
                class="action-icon"
            ...
            <view
              ...
              @click="onApproveClaim(item.claim_id)"
            >
              <image
                class="action-icon"
...
function goToDetail(eventId: string) {
  uni.navigateTo({
    url: `/pages/admin/audit-detail/index?event_id=${eventId}`,
  });
}
